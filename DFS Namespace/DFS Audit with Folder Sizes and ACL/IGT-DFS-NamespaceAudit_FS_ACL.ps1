...
.PARAMETER SkipFolderSize
    Skip recursive folder size and file count calculation for every target path.
    Folder size uses Get-ChildItem -Recurse and can be slow on large shares
    (multi-TB, millions of files). Use this switch for quick structure-only runs
    and omit it for full audits run overnight or off-hours.
...
.EXAMPLE
    ...
    Quick structure-only run — no ACL reads, no recursive size calculation.

.EXAMPLE
    .\IGT-DFS-NamespaceAudit.ps1 -SkipACL -SkipFolderSize
    Fastest possible run — namespace/folder/target structure only.
...
# Fast structure-only run (no ACL, no size crawl)
.\IGT-DFS-NamespaceAudit.ps1 -SkipACL -SkipFolderSize
...
# Size only, no ACL
.\IGT-DFS-NamespaceAudit.ps1 -SkipACL
#>
...
Write-Host "  SkipSize: $($SkipFolderSize.IsPresent)" -ForegroundColor White
Write-Host '  ================================================================' -ForegroundColor Cyan
Write-Host ''

Write-Log "Script started | Server=$NamespaceServer | User=$RunningUser | SkipACL=$($SkipACL.IsPresent) | SkipFolderSize=$($SkipFolderSize.IsPresent)"
...
#-- Recursively calculate total size and file count for a UNC target path.
#   Returns a PSCustomObject with Display (string), Bytes (string), FileCount (string).
#   Storing as strings keeps the $AllRecords schema uniform.
#   Results are cached by the caller via $SizeCache so each unique path is only
#   traversed once even though it appears in multiple ACL records.
function Get-FolderSizeInfo {
    ...
    try {
        ...
        $measured = $items | Measure-Object -Property Length -Sum -ErrorAction SilentlyContinue

        $totalBytes = 0L
        if ($null -ne $measured -and $null -ne $measured.Sum) {
            $totalBytes = [long]$measured.Sum
        }
        $fileCount = 0
        if ($null -ne $measured -and $null -ne $measured.Count) {
            $fileCount = [int]$measured.Count
        }

        return [PSCustomObject]@{
            Display   = Format-FileSize $totalBytes
...
function Export-XlsxReport {
    ...
    $dHdrs = @(
        ...
        'Folder Size','Folder Size (Bytes)','File Count'
    )
    ...
    foreach ($r in $DetailRows) {
        ...
        # T = Folder Size (display), U = Folder Size (bytes, numeric), V = File Count (numeric)
        [void]$sbD.Append("<c r=`"T$rn`" t=`"inlineStr`"><is><t>$(Escape-Xml $r.FolderSizeDisplay)</t></is></c>")
        $bytesLong = 0L
        if ([long]::TryParse($r.FolderSizeBytes, [ref]$bytesLong) -and $bytesLong -ge 0) {
            [void]$sbD.Append("<c r=`"U$rn`" t=`"n`"><v>$bytesLong</v></c>")
        } else {
            [void]$sbD.Append("<c r=`"U$rn`" t=`"inlineStr`"><is><t>$(Escape-Xml $r.FolderSizeBytes)</t></is></c>")
        }
        $fcLong = 0L
        if ([long]::TryParse($r.FileCount, [ref]$fcLong) -and $fcLong -ge 0) {
            [void]$sbD.Append("<c r=`"V$rn`" t=`"n`"><v>$fcLong</v></c>")
        } else {
            [void]$sbD.Append("<c r=`"V$rn`" t=`"inlineStr`"><is><t>$(Escape-Xml $r.FileCount)</t></is></c>")
        }
        [void]$sbD.Append('</row>')
        $rn++
    }
...
function Build-HtmlReport {
    ...
    # Sum folder sizes across unique target paths (avoids double-counting ACL rows)
    $totalSizeBytes   = 0L
    $anySizeCollected = $false
    $uniqueTargetRows = $Records |
        Where-Object { $_.TargetPath -ne '' -and $_.TargetPath -notlike '(No*' } |
        Select-Object -Property TargetPath, FolderSizeBytes -Unique
    foreach ($ut in $uniqueTargetRows) {
        $bval = 0L
        if ([long]::TryParse($ut.FolderSizeBytes, [ref]$bval) -and $bval -ge 0) {
            $totalSizeBytes   += $bval
            $anySizeCollected  = $true
        }
    }
    $totalSizeDisplay = if ($anySizeCollected) { Format-FileSize $totalSizeBytes } else { 'N/A' }
...
.t-size{background:#2a1f0a;color:#e0a84a;border:1px solid #6a4a1a;font-family:'Consolas','Courier New',monospace}
...
<div class="stats-bar">
  ...
  <span class="stat-sep">|</span>
  <div class="stat-item"><span class="stat-n">$totalSizeDisplay</span><span class="stat-lbl">Total Data</span></div>
...
"@)
            ...
            foreach ($tGrp in $tgtGroups) {
                ...
                $tSize    = Escape-Html $tGrp.Group[0].FolderSizeDisplay
                $tFiles   = Escape-Html $tGrp.Group[0].FileCount
                $tId      = "${fId}t${tgtIdx}"
...
"@)
                ...
                if ($tSize -and $tSize -ne 'Skipped' -and $tSize -ne 'Error' -and $tSize -ne '') {
                    $sizeLabel = if ($tFiles -and $tFiles -ne '-1' -and $tFiles -ne '') {
                        "$tSize  |  $tFiles files"
                    } else {
                        $tSize
                    }
                    [void]$sb.Append("              <span class=`"tag t-size`">$sizeLabel</span>`n")
                }
                [void]$sb.Append(@"
            </div>
            <table class="acl-tbl">
...
# Cache folder sizes keyed by target path so each UNC path is only traversed once
$SizeCache        = @{}
...
foreach ($root in $DFSRoots) {
    ...
    foreach ($folder in $folders) {
        ...
        foreach ($target in $fTargets) {
            ...
            # ---- Folder size (cached per unique target path) ----
            if ($SizeCache.ContainsKey($tPath)) {
                $sizeInfo = $SizeCache[$tPath]
            }
            else {
                if (-not $SkipFolderSize) {
                    Write-Host "                       Calculating size..." -ForegroundColor DarkGray -NoNewline
                }
                $sizeInfo = Get-FolderSizeInfo -Path $tPath
                $SizeCache[$tPath] = $sizeInfo
                if (-not $SkipFolderSize) {
                    Write-Host "`r                       Size: $($sizeInfo.Display)  ($($sizeInfo.FileCount) files)          " -ForegroundColor DarkGray
                }
            }
            ...
            foreach ($ace in $aceList) {
                $AllRecords.Add([PSCustomObject]@{
                    ...
                    TargetState        = $tState
                    TargetPriority     = $tPri
                    FolderSizeDisplay  = $sizeInfo.Display
                    FolderSizeBytes    = $sizeInfo.Bytes
...
Write-Host "  Targets    : $($SizeCache.Count) unique target paths scanned"     -ForegroundColor White

# Compute final size total for summary (mirrors HTML builder logic)
$sumBytes   = 0L
$sumHasData = $false
foreach ($sc in $SizeCache.Values) {
    $bv = 0L
    if ([long]::TryParse($sc.Bytes, [ref]$bv) -and $bv -ge 0) {
        $sumBytes   += $bv
        $sumHasData  = $true
    }
}
$sumSizeDisplay = if ($sumHasData) { Format-FileSize $sumBytes } else { 'N/A (skipped or error)' }
Write-Host "  Total Size : $sumSizeDisplay"                                     -ForegroundColor White
