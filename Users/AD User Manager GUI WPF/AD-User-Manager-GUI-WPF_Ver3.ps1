...
.DESCRIPTION
    ...
    Searchable OU TreeView, progress bar, CSV export, locked-account finder.
.NOTES
...
function Copy-ADUserAsTemplate {
    ...
    if ($NewUserData.EmailAddress) { $params.EmailAddress = $NewUserData.EmailAddress }
...
function Get-LockoutOriginMap {
    <#
      Returns a hashtable: SamAccountName -> most recent caller computer name.
      Reads Security event 4740 from the PDC Emulator.
      Note: in event 4740, the "Caller Computer Name" is stored in the
      XML field named 'TargetDomainName'.
    #>
    param([string]$PDC, [PSCredential]$Credential)

    $map = @{}
    $records = @()

    $remoteScript = {
        Get-WinEvent -FilterHashtable @{LogName='Security'; Id=4740} -MaxEvents 3000 -ErrorAction SilentlyContinue |
            ForEach-Object {
                try {
                    $x = [xml]$_.ToXml()
                    $un = ($x.Event.EventData.Data | Where-Object { $_.Name -eq 'TargetUserName'   } | Select-Object -First 1).'#text'
                    $cn = ($x.Event.EventData.Data | Where-Object { $_.Name -eq 'TargetDomainName' } | Select-Object -First 1).'#text'
                    if ($un) { [PSCustomObject]@{ User = $un; Origin = $cn } }
                } catch { }
            }
    }
    ...
    try {
        if ($Credential) {
            $sess = New-PSSession -ComputerName $PDC -Credential $Credential -ErrorAction Stop
            $records = @(Invoke-Command -Session $sess -ScriptBlock $remoteScript -ErrorAction Stop)
            ...
            foreach ($ev in $events) {
                try {
                    ...
                    $un = ($x.Event.EventData.Data | Where-Object { $_.Name -eq 'TargetUserName'   } | Select-Object -First 1).'#text'
                    $cn = ($x.Event.EventData.Data | Where-Object { $_.Name -eq 'TargetDomainName' } | Select-Object -First 1).'#text'
                    if ($un) { $records += [PSCustomObject]@{ User = $un; Origin = $cn } }
                } catch { }
            }
        }
    } catch {
        # Event log unavailable – return empty map (list will show '(unknown)')
    }

    # Events come newest-first; keep the first (most recent) origin per user.
    foreach ($r in $records) {
        if ($r.User -and -not $map.ContainsKey($r.User)) {
            $map[$r.User] = if ($r.Origin) { $r.Origin } else { '(unknown)' }
        }
    }
    return $map
}
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="AD User Manager" Height="820" Width="1140"
      ...
      <TabItem Header="Create User">
        <ScrollViewer VerticalScrollBarVisibility="Auto" HorizontalScrollBarVisibility="Disabled">
          <Grid Margin="4">
            ...
            <!-- Identity: cleaner, one field per row -->
            <GroupBox Header="Identity" Grid.Column="0" Grid.Row="0">
              <Grid>
                ...
                <TextBox   x:Name="txtGiven"     Grid.Row="0" Grid.Column="1"/>

                <TextBlock Text="Last Name *"    Grid.Row="1" VerticalAlignment="Center"/>
                <TextBox   x:Name="txtSur"       Grid.Row="1" Grid.Column="1"/>

                <TextBlock Text="Logon Name *"   Grid.Row="2" VerticalAlignment="Center"/>
                <TextBox   x:Name="txtSam"       Grid.Row="2" Grid.Column="1"/>

                <TextBlock Text="Display Name"   Grid.Row="3" VerticalAlignment="Center"/>
                <TextBox   x:Name="txtDisp"      Grid.Row="3" Grid.Column="1"/>

                <TextBlock Text="UPN *"          Grid.Row="4" VerticalAlignment="Center"/>
                <TextBox   x:Name="txtUPN"       Grid.Row="4" Grid.Column="1"/>

                <TextBlock Text="Email Address"  Grid.Row="5" VerticalAlignment="Center"/>
                <TextBox   x:Name="txtEmail"     Grid.Row="5" Grid.Column="1"/>

                <TextBlock Text="Description"    Grid.Row="6" VerticalAlignment="Center"/>
                <TextBox   x:Name="txtDesc"      Grid.Row="6" Grid.Column="1"/>
              </Grid>
            </GroupBox>
            ...
            <GroupBox Header="Organizational Unit *" Grid.Column="1" Grid.Row="0" Grid.RowSpan="2">
              <DockPanel>
                ...
                <TreeView x:Name="tvOU" Height="280"/>
              </DockPanel>
            </GroupBox>
      ...
      <TabItem Header="Manage Users">
        <Grid Margin="6">
          ...
          <StackPanel Orientation="Horizontal" Margin="0,0,0,10">
            ...
            <TextBox x:Name="txtSearch" Width="240" Height="28"/>
            <Button x:Name="btnSearch" Content="Search" Style="{StaticResource PrimaryBtn}"/>
            <Button x:Name="btnFindLocked" Content="Find Locked Users" Style="{StaticResource WarnBtn}"/>
          ...
          <ListView x:Name="lvUsers" Grid.Row="1" SelectionMode="Extended"
            ...
            <ListView.View>
              <GridView>
                <GridViewColumn Header="SAM" Width="130" DisplayMemberBinding="{Binding SamAccountName}"/>
                <GridViewColumn Header="Display Name" Width="170" DisplayMemberBinding="{Binding DisplayName}"/>
                <GridViewColumn Header="Enabled" Width="65" DisplayMemberBinding="{Binding Enabled}"/>
                <GridViewColumn Header="OU" Width="230" DisplayMemberBinding="{Binding OU}"/>
                <GridViewColumn Header="UPN" Width="180" DisplayMemberBinding="{Binding UserPrincipalName}"/>
                <GridViewColumn Header="Lockout Origin" Width="150" DisplayMemberBinding="{Binding LockoutOrigin}"/>
              </GridView>
            </ListView.View>
          </ListView>
          ...
          <WrapPanel Grid.Row="2" Margin="0,10,0,6">
            ...
            <Button x:Name="btnBulkUnlock" Content="Unlock Selected"/>
            ...
            <GroupBox Header="New User Details">
              <Grid>
                ...
                <StackPanel Grid.Column="1" Grid.Row="2" Margin="8,4,0,0">
                  ...
                  <TextBox x:Name="txtDEmail"/>
                </StackPanel>
                <StackPanel Grid.Column="0" Grid.Row="3" Margin="0,4">
                  <TextBlock Text="Password *"/>
                  <PasswordBox x:Name="txtDPwd" Height="30"/>
                </StackPanel>
                <StackPanel Grid.Column="1" Grid.Row="3" Margin="8,4,0,0">
                  <TextBlock Text=" "/>
                ...
                <StackPanel Grid.Column="0" Grid.Row="4" Grid.ColumnSpan="2" Margin="0,8,0,0">
                ...
                <StackPanel Grid.Column="0" Grid.Row="6" Grid.ColumnSpan="2" Margin="0,10,0,0">
...
$btnFindLocked    = Get-Control 'btnFindLocked'
...
$btnBulkUnlock    = Get-Control 'btnBulkUnlock'
...
$txtDEmail        = Get-Control 'txtDEmail'
...
# ---- Auto-fill for the Create tab ----
function Update-DisplayAndLogon {
    if ($txtGiven.Text -and $txtSur.Text) {
        $txtDisp.Text = "$($txtGiven.Text) $($txtSur.Text)".Trim()
        if (-not $txtSam.Tag) {
            $txtSam.Text = ("{0}.{1}" -f `
                $txtGiven.Text.Substring(0,[Math]::Min(1,$txtGiven.Text.Length)), `
                $txtSur.Text).ToLower() -replace '[^a-z0-9.]',''
        }
    }
}
function Update-DefaultUPNAndEmail {
    if (-not $script:CurrentDomain -or -not $txtSam.Text) { return }
    $dns = $script:CurrentDomain.DNSRoot
    if (-not $txtUPN.Tag)   { $txtUPN.Text   = "$($txtSam.Text)@$dns" }
    if (-not $txtEmail.Tag) { $txtEmail.Text = "$($txtSam.Text)@$dns" }
}

$txtGiven.Add_TextChanged({ Update-DisplayAndLogon })
$txtSur.Add_TextChanged({ Update-DisplayAndLogon })

$txtSam.Add_TextChanged({
    $txtSam.Tag = $true
    Update-DefaultUPNAndEmail
})
$txtUPN.Add_TextChanged({   $txtUPN.Tag   = $true })
$txtEmail.Add_TextChanged({ $txtEmail.Tag = $true })
...
$btnLoadDomain.Add_Click({
    ...
    try {
        ...
        # Refresh default UPN / Email if user hasn't overridden them
        Update-DefaultUPNAndEmail
...
$btnCreate.Add_Click({
    ...
    if ([string]::IsNullOrWhiteSpace($txtGiven.Text) -or [string]::IsNullOrWhiteSpace($txtSur.Text) -or
        ...
        [System.Windows.MessageBox]::Show("Fill all required fields (First Name, Last Name, Logon Name, UPN, Password) and select an OU in the tree.")
        return
    }
...
# Find locked users
$btnFindLocked.Add_Click({
    if (-not $script:CurrentDomain) { [System.Windows.MessageBox]::Show("Load a domain first."); return }

    $txtStatus.Text = "Finding locked accounts and resolving lockout origin..."
    $window.Cursor = [System.Windows.Input.Cursors]::Wait
    $lvUsers.Items.Clear()
    $lstMemberOf.Items.Clear()
    Show-Progress -Value 5 -Maximum 100 -Status "Querying locked accounts..."

    try {
        # Search-ADAccount is the supported cmdlet for the constructed LockedOut attribute.
        $sa = @{ LockedOut = $true }
        if ($script:CurrentServer) { $sa.Server = $script:CurrentServer }
        if ($script:Credential)    { $sa.Credential = $script:Credential }

        $lockedRaw = @(Search-ADAccount @sa | Where-Object { $_.ObjectClass -eq 'user' })

        # Re-hydrate each hit as a full ADUser object (Search-ADAccount returns a slim object).
        $users = @()
        foreach ($h in $lockedRaw) {
            try {
                $p = @{
                    Identity    = $h.DistinguishedName
                    Properties  = 'Enabled','DistinguishedName','UserPrincipalName','DisplayName','lockoutTime'
                    ErrorAction = 'Stop'
                }
                if ($script:CurrentServer) { $p.Server = $script:CurrentServer }
                if ($script:Credential)    { $p.Credential = $script:Credential }
                ...
                Write-Warning "Could not refresh $($h.SamAccountName): $($_.Exception.Message)"
            }
        }

        Show-Progress -Value 25 -Maximum 100 -Status "Found $($users.Count) locked account(s). Reading PDC event log..."

        $pdc = $script:CurrentDomain.PDCEmulator
        $originMap = @{}
        if ($users.Count -gt 0 -and $pdc) {
            $originMap = Get-LockoutOriginMap -PDC $pdc -Credential $script:Credential
        }

        Show-Progress -Value 75 -Maximum 100 -Status "Building result list..."

        foreach ($u in $users) {
            $ou = ($u.DistinguishedName -split ',', 2)[1]
            $origin = if ($originMap.ContainsKey($u.SamAccountName)) { $originMap[$u.SamAccountName] } else { '(unknown)' }
            $item = [PSCustomObject]@{
                SamAccountName    = $u.SamAccountName
                DisplayName       = $u.DisplayName
                Enabled           = $u.Enabled
        ...
        Hide-Progress
        $txtStatus.Text = "Found $($users.Count) locked user(s). PDC queried: $pdc"
        Write-ADLog -Action "FindLocked" -Target $script:CurrentDomain.DNSRoot -Result "Success" -Details "Count=$($users.Count)"
    } catch {
        Hide-Progress
        $txtStatus.Text = "Locked user search failed."
        [System.Windows.MessageBox]::Show($_.Exception.Message, "Locked User Search Error")
        Write-ADLog -Action "FindLocked" -Target $script:CurrentDomain.DNSRoot -Result "Failed" -Details $_.Exception.Message
    } finally {
        $window.Cursor = [System.Windows.Input.Cursors]::Arrow
    }
})
...
$btnExport.Add_Click({
    ...
    if ($dlg.ShowDialog()) {
        $lvUsers.Items |
            Select-Object SamAccountName, DisplayName, Enabled, OU, UserPrincipalName, LockoutOrigin |
...
$btnBulkUnlock.Add_Click({
    Invoke-BulkAction -Items @($lvUsers.SelectedItems) -ConfirmMsg "Unlock $($lvUsers.SelectedItems.Count) account(s)?" -ActionName "UnlockAccount" -Action {
        param($item)
        $p = @{ Identity = $item.SamAccountName; ErrorAction = 'Stop' }
        if ($script:CurrentServer) { $p.Server = $script:CurrentServer }
        if ($script:Credential)    { $p.Credential = $script:Credential }
...
$btnDuplicate.Add_Click({
    ...
    $newData = @{
        ...
        EmailAddress          = $txtDEmail.Text.Trim()
