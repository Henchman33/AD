...
foreach ($GPO in $AllGPOs) {
    ...
    $ComputerRights = $GPOXml.GPO.Computer.ExtensionData.Extension.Policy | Where-Object {
        $_.Name -like "Exclude User Log on as a service"
    }
    ...
    if ($ComputerRights -or $UserRights) {
        ...
        $Results += [PSCustomObject]@{
            ...
            GPOId       = $GPO.Id
            LinkedTo    = ($Links -join ", ")
            UserRight   = "Exclude User Log on as a service (SeServiceLogonRight)"
            ConfigScope = @(
...
if ($Results.Count -like 0) {
    "No GPOs found that configure 'Exclude User Log on as a service' (SeServiceLogonRight)." | Out-File $CsvPath -Encoding UTF8
    "No GPOs found that configure 'Exclude User Log on as a service' (SeServiceLogonRight)." | Out-File $XmlPath -Encoding UTF8
    ...
    "Domain: $DomainName`r`nSummary: $($Results.Count) GPO(s) found with 'Exclude User Log on as a service' configured." | Out-File $SummaryPath -Encoding UTF8
}
