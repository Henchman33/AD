...
<h2>Author: Steve McKee IGTPL<h2>
...
try {
    $gpos = Get-GPO -All -ErrorAction SilentlyContinue |
        Select-Object DisplayName, DomainName, GpoStatus, ModificationTime, Id
    $gpos | Export-Csv "$OutputPath\gpos.csv" -NoTypeInformation
    Add-ContentReport "<details><summary>Group Policy Objects</summary>"
    Add-ContentReport (($gpos | Select-Object DisplayName,DomainName,GpoStatus,ModificationTime | ConvertTo-Html -Fragment) -join "`r`n")
    Add-ContentReport "</details>"
}
...
try {
    ...
    foreach ($gpo in $allGpos) {
        try {
            ...
            # Find any nodes containing "NTLM" (case-insensitive) in their InnerText or attributes
            $matchedNodes = @()
            $allNodes = $xml.SelectNodes("//*")
            foreach ($n in $allNodes) {
                try {
                    $inner = if ($n -and $n.InnerText) { $n.InnerText } else { '' }
                    $attrs = ''
                    if ($n.Attributes) {
                        $attrs = ($n.Attributes | ForEach-Object { $_.Value }) -join '; '
                    }
                    if ($inner -match '(?i)NTLM' -or $attrs -match '(?i)NTLM') {
                        $matchedNodes += $n
                    }
                } catch { }
            }
            ...
            if ($matchedNodes.Count -gt 0) {
                # Determine linked OUs by searching gPLink attribute values for this GPO's GUID
                $links = @()
                if (Get-Command Get-ADOrganizationalUnit -ErrorAction SilentlyContinue) {
                    try {
                        # Match with or without braces - normalize GUID string for regex
                ...
                # If no OU links found, also check domain & site-level links via Get-GPOReport (report contains Links)
                if ($links.Count -eq 0) {
                    try {
                        $linkNodes = $xml.SelectNodes("//LinksTo/GPO")
                        foreach ($ln in $linkNodes) {
                            # Each GPO link object may show its target; include whatever text is helpful
                            if ($ln.InnerText -match '(?i)NTLM') {
                                # unlikely; ignore. We want to capture available LinksTo entries
                            }
                        }
                        # Attempt to read LinksTo/Link nodes for LinkTarget attribute
                        $linkNodes2 = $xml.SelectNodes("//LinksTo/Link")
                        foreach ($lnk in $linkNodes2) {
                            try {
                                $target = $lnk.SelectSingleNode("Properties/Path")
                                if ($target) { $links += $target.InnerText }
                            } catch {}
                        }
                    } catch {}
                }

                # Prepare rows for CSV/HTML: each matched node becomes a row with context (parent path, value)
                foreach ($node in $matchedNodes) {
                    # Friendly setting name: prefer 'Name' attribute, else node name, else parent node name
                    $settingName = ''
                    ...
                    # Try to find a value in common attributes or sibling nodes
                    $value = ''
                    if ($node.Attributes) {
                        foreach ($a in $node.Attributes) {
                            if ($a.Name -match '(?i)value|state|configuredvalue|setting') { $value = $a.Value; break }
                        }
                    }
                    if (-not $value) {
                        # look for child nodes that frequently hold the setting's chosen value
                        $candidate = $node.SelectSingleNode(".//State")  # e.g. State node
                        if ($candidate) { $value = $candidate.InnerText }
                    }
                    if (-not $value) {
                        # fallback to node's text (may be descriptive)
                        $value = ($node.InnerText -replace '\s{2,}', ' ').Trim()
                    }

                    # Build context: parent nodes up to two levels
    ...
    # Export to CSV
    $ntlmGpoData | Export-Csv "$OutputPath\gpos_ntlm.csv" -NoTypeInformation -Force

    # Add to HTML report
    Add-ContentReport "<details><summary>NTLM Configuration GPOs</summary>"
    Add-ContentReport "<p>GPOs containing NTLM-related settings, the specific setting/context/value, whether the GPO is enabled, and the OUs it is linked to.</p>"
    ...
    Write-Warning "NTLM GPO analysis failed: $_"
}
#endregion
