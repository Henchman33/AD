...
<head>
    ...
    <style>
        ...
        .search-container { margin: 20px 0; padding: 20px; background-color: #2d2d2d; border-radius: 4px; border-left: 4px solid #4fc3f7; }
        .search-box { width: 100%; max-width: 600px; padding: 12px; font-size: 16px; background-color: #1e1e1e; color: #e0e0e0; border: 2px solid #455a64; border-radius: 4px; }
        .search-box:focus { outline: none; border-color: #4fc3f7; }
        .search-results { margin-top: 15px; color: #81c784; font-weight: bold; }
        .highlight { background-color: #ffeb3b; color: #000; font-weight: bold; padding: 2px 4px; border-radius: 2px; }
        .search-buttons { margin-top: 10px; }
        .search-btn { padding: 8px 16px; background-color: #455a64; color: #e0e0e0; border: none; cursor: pointer; font-size: 14px; border-radius: 4px; margin-right: 10px; }
        .search-btn:hover { background-color: #546e7a; }
    </style>
    <script>
        ...
        function searchReport() {
            ...
            if (searchTerm.length < 2) {
                resultsDiv.innerHTML = '<span style="color: #ffb74d;">Please enter at least 2 characters to search</span>';
                return;
            }
            ...
            sections.forEach(section => {
                ...
                if (textContent.includes(searchTerm)) {
                    // Expand the section if it contains matches
                    section.classList.remove('collapsed-content');
            ...
            if (matchCount > 0) {
                resultsDiv.innerHTML = '<span class="search-results">Found matches in ' + matchCount + ' section(s)</span>';
            } else {
                resultsDiv.innerHTML = '<span style="color: #ef5350;">No matches found for "' + searchTerm + '"</span>';
            }
        }
...
<body>
    ...
    <div style="margin: 20px 0;">
        ...
        <button class="toggle-all" onclick="expandAll()">Expand All Sections</button>
        <button class="toggle-all" onclick="collapseAll()">Collapse All Sections</button>
    </div>
    <div class="search-container">
        <h3 style="margin-top: 0; color: #4fc3f7;">🔍 Search Report</h3>
        ...
        <div class="search-buttons">
            ...
            <button class="search-btn" onclick="clearHighlights()">Clear Results</button>
        </div>
...
$HTML += "<table><tr><th>Group</th><th>Member Name</th><th>Login Name</th><th>Account Type</th><th>Enabled</th><th>Last Logon</th><th>Password Last Set</th></tr>"
            ...
            foreach ($Member in $Members) {
                try {
                    if ($Member.objectClass -eq "user") {
                        ...
                        $HTML += "<tr class='tier0'><td>$GroupName</td><td>$($User.Name)</td><td>$($User.SamAccountName)</td><td>User</td><td>$EnabledStatus</td><td>$LastLogon</td><td>$PwdLastSet</td></tr>"
                    } else {
                        # Add to data collection
                        $AllData.Tier0Accounts += [PSCustomObject]@{
                            Group = $GroupName
                        ...
                        $HTML += "<tr class='tier0'><td>$GroupName</td><td>$($Member.Name)</td><td>$($Member.SamAccountName)</td><td>$($Member.objectClass)</td><td>N/A</td><td>N/A</td><td>N/A</td></tr>"
                    }
                } catch {
                    $HTML += "<tr class='tier0'><td>$GroupName</td><td>$($Member.Name)</td><td>$($Member.SamAccountName)</td><td>Unknown</td><td colspan='3'>Error querying</td></tr>"
                }
            }
        }
    } catch {
        $HTML += "<tr class='tier0'><td>$GroupName</td><td colspan='6'><span class='critical'>Unable to query group</span></td></tr>"
    }
}
$HTML += "</table></div>"
                ...
                foreach ($Member in $Members) {
                    try {
                        if ($Member.objectClass -eq "user") {
                            ...
                            $HTML += "<tr class='tier1'><td>$GroupName</td><td>$($User.Name)</td><td>$($User.SamAccountName)</td><td>User</td><td>$EnabledStatus</td><td>$LastLogon</td><td>$PwdLastSet</td></tr>"
                        } else {
                            # Add to data collection
                            $AllData.Tier1Accounts += [PSCustomObject]@{
                                Group = $GroupName
                            ...
                            $HTML += "<tr class='tier1'><td>$GroupName</td><td>$($Member.Name)</td><td>$($Member.SamAccountName)</td><td>$($Member.objectClass)</td><td>N/A</td><td>N/A</td><td>N/A</td></tr>"
                        }
                    } catch {
                        $HTML += "<tr class='tier1'><td>$GroupName</td><td>$($Member.Name)</td><td>$($Member.SamAccountName)</td><td>Unknown</td><td colspan='3'>Error querying</td></tr>"
                    }
                }
            } else {
                $HTML += "<tr class='tier1'><td>$GroupName</td><td colspan='6'>No members</td></tr>"
            }
        }
    } catch {
        $HTML += "<tr class='tier1'><td>$GroupName</td><td colspan='6'><span class='critical'>Unable to query group</span></td></tr>"
    }
}
$HTML += "</table></div>"
                ...
                foreach ($Member in $Members) {
                    try {
                        if ($Member.objectClass -eq "user") {
                            ...
                            $HTML += "<tr class='tier2'><td>$GroupName</td><td>$($User.Name)</td><td>$($User.SamAccountName)</td><td>User</td><td>$EnabledStatus</td><td>$LastLogon</td><td>$PwdLastSet</td></tr>"
                        } else {
                            # Add to data collection
                            $AllData.Tier2Accounts += [PSCustomObject]@{
                                Group = $GroupName
                            ...
                            $HTML += "<tr class='tier2'><td>$GroupName</td><td>$($Member.Name)</td><td>$($Member.SamAccountName)</td><td>$($Member.objectClass)</td><td>N/A</td><td>N/A</td><td>N/A</td></tr>"
                        }
                    } catch {
                        $HTML += "<tr class='tier2'><td>$GroupName</td><td>$($Member.Name)</td><td>$($Member.SamAccountName)</td><td>Unknown</td><td colspan='3'>Error querying</td></tr>"
                    }
                }
            } else {
                $HTML += "<tr class='tier2'><td>$GroupName</td><td colspan='6'>No members</td></tr>"
            }
        }
    } catch {
        continue
...
if (-not $Tier2Found) {
    $HTML += "<tr class='tier2'><td colspan='7'>No standard Tier 2 groups found. Please customize the script with your organization's Tier 2 group names.</td></tr>"
}
