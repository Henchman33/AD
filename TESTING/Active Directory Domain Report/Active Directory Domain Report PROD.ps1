...
# ─────────────────────────────────────────────────────────────────────────────
#  SECTION 15 – MICROSOFT TIERED ADMINISTRATION MODEL
# ─────────────────────────────────────────────────────────────────────────────
#
#  Detection strategy (multi-signal — any match qualifies an object):
#
#  Tier 0 (Critical / Identity Infrastructure)
#    Accounts  : SamAccountName prefix  t0-  (e.g. MYIGT\t0-accountname)
#    Groups    : Name contains  Tier 0 | T0- | T0_  (e.g. "Tier 0 Operators")
#    OU path   : DistinguishedName contains  OU=Tier 0  |  OU=T0
#    Computers : Domain Controllers + any computer in a Tier 0 OU
#
#  Tier 1 (Server / Application Tier)
#    Accounts  : SamAccountName prefix  t1-
#    Groups    : Name contains  Tier 1 | T1- | T1_
#    OU path   : DistinguishedName contains  OU=Tier 1  |  OU=T1
#    Computers : Member servers (non-DC) in Tier 1 OU, or all Windows Servers
#                if no explicit Tier 1 OU is found
#
#  Tier 2 (Workstation / End-User Tier)
#    Accounts  : SamAccountName prefix  t2-
#    Groups    : Name contains  Tier 2 | T2- | T2_
#    OU path   : DistinguishedName contains  OU=Tier 2  |  OU=T2
#    Computers : Non-server OS objects (workstations)
#
#  Reference : https://learn.microsoft.com/en-us/microsoft-identity-manager/pam/tier-model-for-partitioning-administrative-privileges
#  Reference : https://techcommunity.microsoft.com/blog/coreinfrastructureandsecurityblog/protecting-tier-0-the-modern-way/4052851

$Step++
Write-Step "Collecting Tiered Administration Model (Tier 0 / 1 / 2)" $Step $TotalSteps

try {
    # Pre-compute DC names for exclusion from Tier 1/2 computer lists
    $DCNameSet = @{}
    if ($DomainControllers) {
        $DomainControllers | ForEach-Object { $DCNameSet[$_.'Name'] = $true }
    }

    # ── TIER 0 ──────────────────────────────────────────────────────────────
    $Tier0Users = $AllUsers | Where-Object {
        $_.'SAM Account' -match '(?i)^t0-' -or
        $_.'Distinguished Name' -match '(?i)OU=Tier[\s_-]*0|OU=T0[^a-z0-9]'
    }

    $Tier0Groups = $AllGroups | Where-Object {
        $_.'Group Name' -match '(?i)(Tier[\s_-]*0|T0[-_ ])' -or
        $_.'SAM Account' -match '(?i)^T0[-_]' -or
        $_.'Distinguished Name' -match '(?i)OU=Tier[\s_-]*0|OU=T0[^a-z0-9]'
    }
    ...
    $Tier0SvcAccounts = if ($SPNUsers) {
        $SPNUsers | Where-Object {
            $_.'SAM Account' -match '(?i)^t0-' -or
            $_.'Distinguished Name' -match '(?i)OU=Tier[\s_-]*0|OU=T0[^a-z0-9]'
        }
    } else { @() }

    # Tier 0 computers = all DCs + any computer explicitly in a Tier 0 OU
    $Tier0OUComputers = $AllComputers | Where-Object {
        $_.'Distinguished Name' -match '(?i)OU=Tier[\s_-]*0|OU=T0[^a-z0-9]'
    }
    $Tier0DCObjects = $AllComputers | Where-Object { $DCNameSet.ContainsKey($_.'Computer Name') }
    # Combine, deduplicate by name
    $Tier0CompNames = @{}
    $Tier0Computers = @()
    foreach ($c in @($Tier0DCObjects) + @($Tier0OUComputers)) {
        if ($c -and -not $Tier0CompNames.ContainsKey($c.'Computer Name')) {
            $Tier0CompNames[$c.'Computer Name'] = $true
            $Tier0Computers += $c
        }
    }
    $Tier0Computers = $Tier0Computers | Sort-Object 'Computer Name'

    # ── TIER 1 ──────────────────────────────────────────────────────────────
    $Tier1Users = $AllUsers | Where-Object {
        $_.'SAM Account' -match '(?i)^t1-' -or
        $_.'Distinguished Name' -match '(?i)OU=Tier[\s_-]*1|OU=T1[^a-z0-9]'
    }

    $Tier1Groups = $AllGroups | Where-Object {
        $_.'Group Name' -match '(?i)(Tier[\s_-]*1|T1[-_ ])' -or
        $_.'SAM Account' -match '(?i)^T1[-_]' -or
        $_.'Distinguished Name' -match '(?i)OU=Tier[\s_-]*1|OU=T1[^a-z0-9]'
    }
    ...
    # Tier 1 computers — explicit Tier 1 OU first; fallback to all member servers
    $Tier1OUComputers = $AllComputers | Where-Object {
        $_.'Distinguished Name' -match '(?i)OU=Tier[\s_-]*1|OU=T1[^a-z0-9]' -and
        -not $DCNameSet.ContainsKey($_.'Computer Name')
    }
    # If no Tier 1 OUs found, use all Windows Server computers that are NOT DCs
    if (-not $Tier1OUComputers -or $Tier1OUComputers.Count -eq 0) {
        $Tier1Computers = $AllComputers | Where-Object {
            $_.'Operating System' -match 'Server' -and
            -not $DCNameSet.ContainsKey($_.'Computer Name')
        } | Sort-Object 'Computer Name'
    } else {
        $Tier1Computers = $Tier1OUComputers | Sort-Object 'Computer Name'
    }

    # ── TIER 2 ──────────────────────────────────────────────────────────────
    $Tier2Users = $AllUsers | Where-Object {
        $_.'SAM Account' -match '(?i)^t2-' -or
        $_.'Distinguished Name' -match '(?i)OU=Tier[\s_-]*2|OU=T2[^a-z0-9]'
    }

    $Tier2Groups = $AllGroups | Where-Object {
        $_.'Group Name' -match '(?i)(Tier[\s_-]*2|T2[-_ ])' -or
        $_.'SAM Account' -match '(?i)^T2[-_]' -or
        $_.'Distinguished Name' -match '(?i)OU=Tier[\s_-]*2|OU=T2[^a-z0-9]'
    }

    # Tier 2 computers — explicit OU first; fallback to non-server, non-DC
    $Tier2OUComputers = $AllComputers | Where-Object {
        $_.'Distinguished Name' -match '(?i)OU=Tier[\s_-]*2|OU=T2[^a-z0-9]' -and
        -not $DCNameSet.ContainsKey($_.'Computer Name')
    }
    if (-not $Tier2OUComputers -or $Tier2OUComputers.Count -eq 0) {
        $Tier2Computers = $AllComputers | Where-Object {
            $_.'Operating System' -notmatch 'Server' -and
            -not $DCNameSet.ContainsKey($_.'Computer Name')
        } | Sort-Object 'Computer Name'
    } else {
        $Tier2Computers = $Tier2OUComputers | Sort-Object 'Computer Name'
    }

    # ── EXPORT CSVs ─────────────────────────────────────────────────────────
    Export-SafeCsv $Tier0Users       (Join-Path $CsvFolder "15-Tier0-Users.csv")           "Tier 0 Users"
    Export-SafeCsv $Tier0Groups      (Join-Path $CsvFolder "15-Tier0-Groups.csv")          "Tier 0 Groups"
    Export-SafeCsv $Tier0MSA         (Join-Path $CsvFolder "15-Tier0-MSA.csv")             "Tier 0 MSA/gMSA"
    Export-SafeCsv $Tier0SvcAccounts (Join-Path $CsvFolder "15-Tier0-ServiceAccounts.csv") "Tier 0 Svc Accounts"
    Export-SafeCsv $Tier0Computers   (Join-Path $CsvFolder "15-Tier0-Computers.csv")       "Tier 0 Computers"
    Export-SafeCsv $Tier1Users       (Join-Path $CsvFolder "15-Tier1-Users.csv")           "Tier 1 Users"
    Export-SafeCsv $Tier1Groups      (Join-Path $CsvFolder "15-Tier1-Groups.csv")          "Tier 1 Groups"
    Export-SafeCsv $Tier1MSA         (Join-Path $CsvFolder "15-Tier1-MSA.csv")             "Tier 1 MSA/gMSA"
    Export-SafeCsv $Tier1SvcAccounts (Join-Path $CsvFolder "15-Tier1-ServiceAccounts.csv") "Tier 1 Svc Accounts"
    Export-SafeCsv $Tier1Computers   (Join-Path $CsvFolder "15-Tier1-Computers.csv")       "Tier 1 Computers"
    Export-SafeCsv $Tier2Users       (Join-Path $CsvFolder "15-Tier2-Users.csv")           "Tier 2 Users"
    Export-SafeCsv $Tier2Groups      (Join-Path $CsvFolder "15-Tier2-Groups.csv")          "Tier 2 Groups"
    Export-SafeCsv $Tier2Computers   (Join-Path $CsvFolder "15-Tier2-Computers.csv")       "Tier 2 Computers"

} catch {
    Write-Warning "Tiered administration collection failed: $_"
    $Tier0Users = @(); $Tier0Groups = @(); $Tier0MSA = @()
    $Tier0SvcAccounts = @(); $Tier0Computers = @()
    $Tier1Users = @(); $Tier1Groups = @(); $Tier1MSA = @()
    $Tier1SvcAccounts = @(); $Tier1Computers = @()
    $Tier2Users = @(); $Tier2Groups = @(); $Tier2Computers = @()
}
...
# Tiered Administration counts
$totalTier0Users  = if ($Tier0Users)       { $Tier0Users.Count }       else { 0 }
$totalTier0Groups = if ($Tier0Groups)      { $Tier0Groups.Count }      else { 0 }
$totalTier0MSA    = if ($Tier0MSA)         { $Tier0MSA.Count }         else { 0 }
$totalTier0SvcAcc = if ($Tier0SvcAccounts) { $Tier0SvcAccounts.Count } else { 0 }
$totalTier0Comp   = if ($Tier0Computers)   { $Tier0Computers.Count }   else { 0 }
$totalTier1Users  = if ($Tier1Users)       { $Tier1Users.Count }       else { 0 }
$totalTier1Groups = if ($Tier1Groups)      { $Tier1Groups.Count }      else { 0 }
$totalTier1MSA    = if ($Tier1MSA)         { $Tier1MSA.Count }         else { 0 }
$totalTier1SvcAcc = if ($Tier1SvcAccounts) { $Tier1SvcAccounts.Count } else { 0 }
$totalTier1Comp   = if ($Tier1Computers)   { $Tier1Computers.Count }   else { 0 }
$totalTier2Users  = if ($Tier2Users)       { $Tier2Users.Count }       else { 0 }
$totalTier2Groups = if ($Tier2Groups)      { $Tier2Groups.Count }      else { 0 }
$totalTier2Comp   = if ($Tier2Computers)   { $Tier2Computers.Count }   else { 0 }
...
$T_Tier0Users   = ConvertTo-HtmlTable $Tier0Users       "tbl-t0users"
$T_Tier0Groups  = ConvertTo-HtmlTable $Tier0Groups      "tbl-t0groups"
$T_Tier0MSA     = ConvertTo-HtmlTable $Tier0MSA         "tbl-t0msa"
$T_Tier0SvcAcc  = ConvertTo-HtmlTable $Tier0SvcAccounts "tbl-t0svcacc"
$T_Tier0Comps   = ConvertTo-HtmlTable $Tier0Computers   "tbl-t0comps"
$T_Tier1Users   = ConvertTo-HtmlTable $Tier1Users       "tbl-t1users"
$T_Tier1Groups  = ConvertTo-HtmlTable $Tier1Groups      "tbl-t1groups"
$T_Tier1MSA     = ConvertTo-HtmlTable $Tier1MSA         "tbl-t1msa"
$T_Tier1SvcAcc  = ConvertTo-HtmlTable $Tier1SvcAccounts "tbl-t1svcacc"
$T_Tier1Comps   = ConvertTo-HtmlTable $Tier1Computers   "tbl-t1comps"
$T_Tier2Users   = ConvertTo-HtmlTable $Tier2Users       "tbl-t2users"
$T_Tier2Groups  = ConvertTo-HtmlTable $Tier2Groups      "tbl-t2groups"
$T_Tier2Comps   = ConvertTo-HtmlTable $Tier2Computers   "tbl-t2comps"
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── TIER BADGES ── */
  .tier-t0 { background: rgba(210,153,34,.18); color: #f5c842; border: 1px solid rgba(210,153,34,.4); }
  .tier-t1 { background: rgba(56,139,253,.15); color: #79b8ff; border: 1px solid rgba(56,139,253,.35); }
  .tier-t2 { background: rgba(46,160,67,.15);  color: #56d364; border: 1px solid rgba(46,160,67,.35); }
  .tier-label {
    display: inline-flex; align-items: center; gap: 6px;
    padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 700;
    text-transform: uppercase; letter-spacing: .5px;
  }
  /* Tier section header accent lines */
  .section-card.tier-0 .section-header { border-left: 3px solid #f5c842; }
  .section-card.tier-1 .section-header { border-left: 3px solid #79b8ff; }
  .section-card.tier-2 .section-header { border-left: 3px solid #56d364; }
  .section-card.tier-0 .section-count  { background: rgba(210,153,34,.15); color: #f5c842; }
  .section-card.tier-1 .section-count  { background: rgba(56,139,253,.15); color: #79b8ff; }
  .section-card.tier-2 .section-count  { background: rgba(46,160,67,.15);  color: #56d364; }
  /* Tier stat cards */
  .stat-card.tier-0 .stat-value { color: #f5c842; }
  .stat-card.tier-1 .stat-value { color: #79b8ff; }
  .stat-card.tier-2 .stat-value { color: #56d364; }
  .stat-card.tier-0 { border-left: 3px solid #f5c842; }
  .stat-card.tier-1 { border-left: 3px solid #79b8ff; }
  .stat-card.tier-2 { border-left: 3px solid #56d364; }
  /* Tier overview grid */
  .tier-overview {
    display: grid;
  ...
  .tier-panel.t0 { background: rgba(210,153,34,.06); border-color: rgba(210,153,34,.3); }
  .tier-panel.t1 { background: rgba(56,139,253,.06); border-color: rgba(56,139,253,.3); }
  .tier-panel.t2 { background: rgba(46,160,67,.06);  border-color: rgba(46,160,67,.3); }
  .tier-panel h4 { font-size: 13px; font-weight: 700; margin-bottom: 10px; }
  .tier-panel.t0 h4 { color: #f5c842; }
  .tier-panel.t1 h4 { color: #79b8ff; }
  .tier-panel.t2 h4 { color: #56d364; }
  .tier-stat-row { display: flex; justify-content: space-between; font-size: 12px; padding: 3px 0; border-bottom: 1px solid rgba(255,255,255,.04); }
  .tier-stat-row:last-child { border-bottom: none; }
  .tier-stat-row span:last-child { font-weight: 700; }
...
<section class="stats-grid">
  ...
  <div class="stat-card tier-0"><div class="stat-label">&#x1F451; Tier 0 Objects</div><div class="stat-value">$($totalTier0Users + $totalTier0Groups + $totalTier0MSA + $totalTier0Comp)</div><div class="stat-sub">$totalTier0Users users / $totalTier0Comp computers</div></div>
  <div class="stat-card tier-1"><div class="stat-label">&#x1F5A5; Tier 1 Objects</div><div class="stat-value">$($totalTier1Users + $totalTier1Groups + $totalTier1MSA + $totalTier1Comp)</div><div class="stat-sub">$totalTier1Users users / $totalTier1Comp computers</div></div>
  <div class="stat-card tier-2"><div class="stat-label">&#x1F4BB; Tier 2 Objects</div><div class="stat-value">$($totalTier2Users + $totalTier2Groups + $totalTier2Comp)</div><div class="stat-sub">$totalTier2Users users / $totalTier2Comp computers</div></div>
</section>
...
<!-- ═══ 15. TIERED ADMINISTRATION MODEL ═══ -->
<!-- ─── TIER 0 ─── -->
<div class="section-card tier-0" id="sec-tier0">
  <div class="section-header" onclick="toggleSection(this)">
    <span class="section-icon">&#x1F451;</span>
    <span class="section-title">Tier 0 — Identity &amp; Domain Infrastructure</span>
    <span class="section-count">$($totalTier0Users + $totalTier0Groups + $totalTier0MSA + $totalTier0SvcAcc + $totalTier0Comp) objects detected</span>
    <span class="chevron">&#9654;</span>
  </div>
  <div class="section-body">

    <div class="alert warn">&#x1F512; <strong>Tier 0 is the most critical tier.</strong>
    These accounts have direct or indirect control over all AD-integrated identities and systems — including Domain Controllers, PKI, virtualisation hosts running DCs, and identity management platforms.
    Strict PAW (Privileged Access Workstation) enforcement and Authentication Policy Silos should be applied to all Tier 0 accounts.
    Reference: <em>Microsoft Enterprise Access Model / AD Administrative Tier Model</em></div>

    <div class="tier-overview">
      <div class="tier-panel t0">
        <h4>&#x1F451; Tier 0 Summary</h4>
        <div class="tier-stat-row"><span>Admin User Accounts</span><span>$totalTier0Users</span></div>
        <div class="tier-stat-row"><span>Security Groups</span><span>$totalTier0Groups</span></div>
        <div class="tier-stat-row"><span>MSA / gMSA Accounts</span><span>$totalTier0MSA</span></div>
        <div class="tier-stat-row"><span>SPN Service Accounts</span><span>$totalTier0SvcAcc</span></div>
        <div class="tier-stat-row"><span>Computers (DCs + T0 OU)</span><span>$totalTier0Comp</span></div>
      </div>
      <div class="tier-panel t0" style="grid-column: span 2;">
        <h4>Detection Criteria Applied</h4>
        <div class="tier-stat-row"><span>Account SAM prefix</span><span style="font-family:monospace">t0-*  (e.g. MYIGT\t0-accountname)</span></div>
        <div class="tier-stat-row"><span>Group name pattern</span><span style="font-family:monospace">"Tier 0 *" / "T0-*" / "T0_*"</span></div>
        <div class="tier-stat-row"><span>OU path pattern</span><span style="font-family:monospace">OU=Tier 0 / OU=T0</span></div>
        <div class="tier-stat-row"><span>Computers</span><span>All Domain Controllers + explicit Tier 0 OU members</span></div>
      </div>
    </div>

    <div class="subsection"><div class="subsection-title"><span class="tier-label tier-t0">&#x1F451; T0</span> User Accounts ($totalTier0Users)</div>$T_Tier0Users</div>
    <div class="subsection"><div class="subsection-title"><span class="tier-label tier-t0">&#x1F451; T0</span> Security Groups ($totalTier0Groups)</div>$T_Tier0Groups</div>
    <div class="subsection"><div class="subsection-title"><span class="tier-label tier-t0">&#x1F451; T0</span> MSA / gMSA Accounts ($totalTier0MSA)</div>$T_Tier0MSA</div>
    <div class="subsection"><div class="subsection-title"><span class="tier-label tier-t0">&#x1F451; T0</span> SPN Service Accounts ($totalTier0SvcAcc)</div>$T_Tier0SvcAcc</div>
    <div class="subsection"><div class="subsection-title"><span class="tier-label tier-t0">&#x1F451; T0</span> Computers — Domain Controllers &amp; Tier 0 OU ($totalTier0Comp)</div>$T_Tier0Comps</div>
  </div>
</div>

<!-- ─── TIER 1 ─── -->
<div class="section-card tier-1" id="sec-tier1">
  <div class="section-header" onclick="toggleSection(this)">
    <span class="section-icon">&#x1F5A5;&#xFE0F;</span>
    <span class="section-title">Tier 1 — Server &amp; Application Infrastructure</span>
    <span class="section-count">$($totalTier1Users + $totalTier1Groups + $totalTier1MSA + $totalTier1SvcAcc + $totalTier1Comp) objects detected</span>
    <span class="chevron">&#9654;</span>
  </div>
  <div class="section-body">

    <div class="alert info">&#x2139;&#xFE0F; <strong>Tier 1 covers member server infrastructure.</strong>
    Tier 1 admin accounts must NEVER be used to log on to Tier 0 systems, and Tier 1 credentials must not be exposed to Tier 2 (workstation) systems.
    Server admins should use dedicated Tier 1 accounts (t1-*) exclusively from approved jump servers or PAWs.</div>

    <div class="tier-overview">
      <div class="tier-panel t1">
        <h4>&#x1F5A5; Tier 1 Summary</h4>
        <div class="tier-stat-row"><span>Admin User Accounts</span><span>$totalTier1Users</span></div>
        <div class="tier-stat-row"><span>Security Groups</span><span>$totalTier1Groups</span></div>
        <div class="tier-stat-row"><span>MSA / gMSA Accounts</span><span>$totalTier1MSA</span></div>
        <div class="tier-stat-row"><span>SPN Service Accounts</span><span>$totalTier1SvcAcc</span></div>
        <div class="tier-stat-row"><span>Computers (Servers)</span><span>$totalTier1Comp</span></div>
      </div>
      <div class="tier-panel t1" style="grid-column: span 2;">
        <h4>Detection Criteria Applied</h4>
        <div class="tier-stat-row"><span>Account SAM prefix</span><span style="font-family:monospace">t1-*  (e.g. MYIGT\t1-accountname)</span></div>
        <div class="tier-stat-row"><span>Group name pattern</span><span style="font-family:monospace">"Tier 1 *" / "T1-*" / "T1_*"</span></div>
        <div class="tier-stat-row"><span>OU path pattern</span><span style="font-family:monospace">OU=Tier 1 / OU=T1</span></div>
        <div class="tier-stat-row"><span>Computers (fallback)</span><span>All Windows Server objects excluding Domain Controllers</span></div>
      </div>
    </div>

    <div class="subsection"><div class="subsection-title"><span class="tier-label tier-t1">&#x1F5A5; T1</span> User Accounts ($totalTier1Users)</div>$T_Tier1Users</div>
    <div class="subsection"><div class="subsection-title"><span class="tier-label tier-t1">&#x1F5A5; T1</span> Security Groups ($totalTier1Groups)</div>$T_Tier1Groups</div>
    <div class="subsection"><div class="subsection-title"><span class="tier-label tier-t1">&#x1F5A5; T1</span> MSA / gMSA Accounts ($totalTier1MSA)</div>$T_Tier1MSA</div>
    <div class="subsection"><div class="subsection-title"><span class="tier-label tier-t1">&#x1F5A5; T1</span> SPN Service Accounts ($totalTier1SvcAcc)</div>$T_Tier1SvcAcc</div>
    <div class="subsection"><div class="subsection-title"><span class="tier-label tier-t1">&#x1F5A5; T1</span> Computers — Member Servers ($totalTier1Comp)</div>$T_Tier1Comps</div>
  </div>
</div>

<!-- ─── TIER 2 ─── -->
<div class="section-card tier-2" id="sec-tier2">
  <div class="section-header" onclick="toggleSection(this)">
    <span class="section-icon">&#x1F4BB;</span>
    <span class="section-title">Tier 2 — Workstation &amp; End-User Devices</span>
    <span class="section-count">$($totalTier2Users + $totalTier2Groups + $totalTier2Comp) objects detected</span>
    <span class="chevron">&#9654;</span>
  </div>
  <div class="section-body">

    <div class="alert info">&#x2139;&#xFE0F; <strong>Tier 2 is the end-user and workstation tier.</strong>
    Helpdesk and workstation admin accounts (t2-*) should only have access to Tier 2 systems and must never be permitted to authenticate to Tier 0 or Tier 1 assets.
    Standard user accounts (no tier prefix) are typically Tier 2 subjects, not Tier 2 administrators.</div>

    <div class="subsection"><div class="subsection-title"><span class="tier-label tier-t2">&#x1F4BB; T2</span> User Accounts ($totalTier2Users)</div>$T_Tier2Users</div>
    <div class="subsection"><div class="subsection-title"><span class="tier-label tier-t2">&#x1F4BB; T2</span> Security Groups ($totalTier2Groups)</div>$T_Tier2Groups</div>
    <div class="subsection"><div class="subsection-title"><span class="tier-label tier-t2">&#x1F4BB; T2</span> Computers — Workstations ($totalTier2Comp)</div>$T_Tier2Comps</div>
  </div>
</div>
    ...
    if (text.includes(q)) {
      ...
      row.querySelectorAll('td').forEach(cell => {
        if (cell.textContent.toLowerCase().includes(q)) {
          ...
          cell.innerHTML = cell.innerHTML.replace(re, '<span class="highlight">`$1</span>');
        }
      });
    }
  });
...
Write-Host ""
Write-Host "  Tiered Administration Model:" -ForegroundColor Gray
Write-Host "    Tier 0 Users       : $totalTier0Users  | Groups: $totalTier0Groups  | MSA: $totalTier0MSA  | Computers: $totalTier0Comp" -ForegroundColor $(if(($totalTier0Users + $totalTier0Groups) -gt 0){'Cyan'}else{'White'})
Write-Host "    Tier 1 Users       : $totalTier1Users  | Groups: $totalTier1Groups  | MSA: $totalTier1MSA  | Computers: $totalTier1Comp" -ForegroundColor $(if(($totalTier1Users + $totalTier1Groups) -gt 0){'Cyan'}else{'White'})
Write-Host "    Tier 2 Users       : $totalTier2Users  | Groups: $totalTier2Groups  | Computers: $totalTier2Comp" -ForegroundColor White
Write-Host ""
