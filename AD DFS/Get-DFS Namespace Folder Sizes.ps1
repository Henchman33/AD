...
.PARAMETER SkipFolderSize
    Skip recursive folder size / file count calculation for each target path.
    NOTE: Folder size collection recurses every target path with Get-ChildItem and
    can be SLOW on large shares (multi-TB / millions of files). Use this switch
    for a quick run, or run overnight / off-hours for full size totals.
...
#-- Recursively calculate total size and file count for a target UNC path.
#   Results are cached by the caller ($SizeCache) since the same target path
#   repeats once per ACL entry.
function Get-FolderSizeInfo {
    ...
    try {
        ...
        $bytes = 0
        if ($stats.Sum) { $bytes = [long]$stats.Sum }

        return [PSCustomObject]@{
            Bytes     = $bytes
            Display   = (Format-FileSize $bytes)
...
function Export-XlsxReport {
    ...
    foreach ($r in $DetailRows) {
        ...
        if ($r.FolderSizeBytes -is [long] -or $r.FolderSizeBytes -is [int]) {
            [void]$sbD.Append("<c r=`"U$rn`" t=`"n`"><v>$($r.FolderSizeBytes)</v></c>")
        } else {
            [void]$sbD.Append("<c r=`"U$rn`" t=`"inlineStr`"><is><t>$(Escape-Xml $r.FolderSizeBytes)</t></is></c>")
        }
        if ($r.FileCount -is [int]) {
            [void]$sbD.Append("<c r=`"V$rn`" t=`"n`"><v>$($r.FileCount)</v></c>")
        } else {
...
function Build-HtmlReport {
    ...
    # Total size across unique target paths (skip non-numeric markers)
    $uniqueTargets = $Records | Where-Object { $_.TargetPath -ne '' -and $_.TargetPath -notlike '(No*' } |
                     Select-Object -Property TargetPath, FolderSizeBytes -Unique
    $totalBytes = 0
    $sizeSkipped = $false
    foreach ($t in $uniqueTargets) {
        if ($t.FolderSizeBytes -is [long] -or $t.FolderSizeBytes -is [int]) {
            if ($t.FolderSizeBytes -ge 0) { $totalBytes += $t.FolderSizeBytes }
        }
        else { $sizeSkipped = $true }
    }
    $totalSizeDisplay = if ($sizeSkipped -and $totalBytes -eq 0) { 'N/A' } else { Format-FileSize $totalBytes }
...
.t-size{background:var(--amber-bg);color:var(--amber);border:1px solid #6a4a1a}
...
foreach ($root in $DFSRoots) {
    ...
    foreach ($folder in $folders) {
        ...
        foreach ($target in $fTargets) {
            ...
            # Calculate folder size / file count (cached per unique target path)
